...
import { existsSync, mkdirSync, readdirSync, readFileSync, rm, rmSync, writeFileSync, } from "node:fs";
...
async function pack(packageDir) {
    ...
    await new Promise((resolve, reject) => {
        ...
        const p = spawn("npm", ["pack", "--pack-destination", LPCK_PACK_DIR, "--workspaces"], { stdio: ["ignore", "ignore", "ignore"], cwd: packageDir });
        p.on("exit", (code) => (code === 0 ? resolve() : reject(code)));
    });
}
async function installAllPacks(packageDir, rawInstall) {
    if (!existsSync(LPCK_PACK_DIR)) {
        return;
    }
    ...
    await new Promise((resolve, reject) => {
        const p = spawn("npm", ["install", ...(rawInstall ? [] : tgzPaths), "--no-save"], {
            stdio: "inherit",
            cwd: packageDir,
        });
...
function getPackName(packageJson) {
...
class WorkspaceHandler {
    ...
    async load(includeWorkspaceRoot = true) {
        ...
        for (const [key, value] of map.entries()) {
            ...
            this.#packagesInfos.set(key, {
                ...
                oldDevDependencies: structuredClone(packageJson.content.devDependencies ?? {}),
                oldPeerDependencies: structuredClone(packageJson.content.peerDependencies ?? {}),
            });
        }
        ...
        if (includeWorkspaceRoot) {
            this.#packagesInfos.set(getPackageName(this.#rootPackage), {
                ...
                oldDependencies: structuredClone(this.#rootPackage.content.dependencies ?? {}),
                oldDevDependencies: structuredClone(this.#rootPackage.content.devDependencies ?? {}),
                oldPeerDependencies: structuredClone(this.#rootPackage.content.peerDependencies ?? {}),
            });
        }
    }
    isWorkspace() {
    ...
    getPackages() {
        return Array.from(this.#packagesInfos.values()).map((packageInfo) => packageInfo.packageJson);
    }
    getRootPackage() {
        if (!this.#rootPackage) {
    ...
    async updateToLocalPacks(availablePackages, options) {
        ...
        const availableDependencies = availablePackages
            ? availablePackages
        ...
        for (const packageInfo of this.#packagesInfos.values()) {
            ...
            for (const depType of depTypes) {
                ...
                for (const dependencyName of Object.keys(newDeps)) {
                    const depToUse = availableDependencies.find((pck) => pck.name === dependencyName);
                    if (depToUse) {
...
class OriginWorkspace {
    ...
    async load() {
        ...
        console.info("Origin package root loaded: ", bold(getPackageName(rootPackage)));
        console.info("Loading workspaces...");
        console.info("Workspaces loaded: ", dim(String(this.#workspaceHandler.getPackages().length)));
    }
...
class TargetWorkspace {
    ...
    async install(availablePackages, options) {
        await this.#workspaceHandler.updateToLocalPacks(availablePackages, options);
        await installAllPacks(this.#workspaceHandler.getRootDir(), options.rawInstall);
...
class LPCK {
    ...
    #loadArgs(args) {
        ...
        const { preset, printPresets, init, prepack, prepackCmd, rawInstall, dev, peer, clean, addPreset, } = parsedArgs.values;
    ...
    #help() {
        console.info("Usage:", code("lpck <workspace-root-package-dir> [more-workspace-root-package-dirs...]"), "or", code("lpck [options]"));
    ...
    async #validatePresetPath(presetPath) {
        try {
    ...
    async #addPreset(name, presetPaths) {
        if (!name) {
            console.error("Missing preset name. Usage:", code("lpck --addPreset <name> <path> [more-paths...]"));
            process.exit(1);
            return;
        }
        if (!presetPaths || presetPaths.length === 0) {
            console.error("Missing preset path. Usage:", code("lpck --addPreset <name> <path> [more-paths...]"));
            process.exit(1);
            return;
        }
        ...
        for (const resolvedPath of resolvedPaths) {
            ...
            if (!isValidPresetPath) {
                console.error("Preset path must contain a package.json with name and version, or a workspace root with workspaces:", code(resolvedPath));
                process.exit(1);
            }
        }
        const presetIndex = this.#lpckRc.presets.findIndex((preset) => preset.name === name);
        const presetToSave = {
            name,
        ...
        console.info(presetIndex >= 0 ? "Preset updated:" : "Preset added:", code(name));
    }
    ...
    async #installAll(originPackageDirs) {
