...
class LPCK {
    ...
    async #addPreset(name, presetPath) {
        ...
        if (!isValidPresetPath) {
            console.error("Preset path must contain a valid npm package or workspace root:", code(resolvedPath));
            process.exit(1);
            return;
        }
        const existingPresetIndex = this.#lpckRc.presets.findIndex((preset) => preset.name === name);
        const presetToSave = {
            name,
            path: resolvedPath,
        ...
        console.info(existingPresetIndex >= 0 ? "Preset updated:" : "Preset added:", code(name));
    }
